...
/**
 * This function returns a timestamp file name which can be used to store all changes made for a particular commit
 ...
 **/
	...
	var changelogfile = this.logfile;
	var appendLogs = function(data) {
	    fsys.appendFile(changeLogPath + changelogfile,
			    JSON.stringify(data),
			    function(err){ if(err) { throw err; }});
	};
	getChanges(filepath,changeLogPath+filepath,appendLogs);
    };
...
**/
	var checkEndOfProcessing = function(length) {
		...
		var commitAll = function(afterCommit) {
		    ...
		    filesToChange.map(addChangesToLog, {"logfile":logfilename});
		    filesToChange.map(copyFileToRepository);
		    afterCommit();
		};
 ...
 **/
	var processFiles = function(f){
		...
		if(exists) {
		    // Need to add logic to check for file changed status
		    // before pushing it into filesChanged list
