...
var writeChangesToFile = function (filename,fileChanges) {
    var changedLogFile = function(err) {
	if(!err) {
	    console.log("changed log file");
	}
    };
    var writeLog = function(data) {
	fsys.appendFile(filename,data,changedLogFile);
    };
    getChanges(filename, changeLogPath+filename,writeLog);
}
/**
 ...
 **/
     var copyFileToRepository = function (filepath) {
	 copyFile(filepath,changeLogPath+filepath,function(){
	    console.log("Added file %s to repository",filepath);
	});
    };


    var addChangesToLog = function(filepath) {
	var changeFilename = "changes.chg";
	writeChangesToFile(changeLogPath+changeFilename,getChanges(filepath,changeLogPath+filepath));
    }
/**
...
**/
	var checkEndOfProcessing = function(length) {
		...
		var commitAll = function(callback) {
		    console.log("Commiting all changes");
		    console.log(filesToChange.concat(filesToAdd).join("\n"));
		    filesToAdd.map(copyFileToRepository);
		    filesToChange.map(addChangesToLog);
		    callback();
		};
